...
\begin{algorithm}
  ...
  \begin{algorithmic}
    ...
    \item[] % blank line
    \Function{CountNodalDomainsInterp}{$grid, counted, \alpha, M, \rho$}
    ...
    \item[]
    \Function{Upsample}{$grid, \alpha, M, \rho$}
    ...
    \item[]
    \Function{Interpolate}{$grid, upsampled, interp, i, j$}
...
\begin{algorithm}
  ...
  \begin{algorithmic}
    ...
    \Function{FindDomain}{$upsampled, counted, i, j, interp$}
        \State $s.push(j,i)$
        \State $counted[i][j] = COUNTED$
        \Comment $s$ is a stack
        \State $sign \gets sign(upsampled[i][j])$
        \While{$x,y \gets s.pop()$}
            \If{$sign(upsampled(x-1,y)) = sign$ and $counted[x-1][y] = UNCOUNTED$} \Comment check point to the $left$
