...
\newtheorem{mean_conj}{Conjecture}
\begin{mean_conj}[Mean of Nodal Domain Count]
...
\newtheorem{var_conj}{Conjecture}
\begin{var_conj}[Variance of Nodal Domain Count]
...
\newtheorem{area_conj}{Conjecture}
\begin{area_conj}[Area of Nodal Domains]
...
For both billiards considered here $B \sim k$. Thus, in practice $N \gg n$ and $N \gg B$ so it is primarily $N$ (which scales like $\alpha^{-2}$) that determines the time to compute eigenfunctions. Additionally, we find that the time to evaluate eigenfunctions dominates total computation time (Fig. \ref{fig:timing}).
...
\begin{figure}
  \begin{center}
    \includegraphics[width=\textwidth]{figs/results/interp_counts.eps}
    \caption{Number of interpolations per pixel}
    \label{fig:interp_counts}
  \end{center}
\end{figure}
